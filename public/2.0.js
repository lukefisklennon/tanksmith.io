...
var go = function(ip) {
	try {
		...
		if (mobile) {
			...
			var joystickPower = 0;
			var joystickSize = 60 * mobileScale;
			var joystickAlpha = 0;
		...
		function rescale() {
			...
			// // Before cs is changed to reflect server serverScale
			// middleContainer.style["transform"] = "translate(-50%, -50%) scale(" + cs * menuScale + ")";
			// for (var i = 0; i < hudElements.length; i++) {
			// 	var transformRule = "";
			// 	try {
			// 		transformRule = (css(hudElements[i]).join(" ").split("transform: ")[1].split(";")[0]) + " ";
			// 	} catch(e) {};
			// 	hudElements[i].style["transform"] = transformRule + "scale(" + cs * menuScale + ")";
			// 	hudElements[i].style["margin"] = (cs * 10) + "px";
			// }
			// socialCard.style["transform"] = "scale(" + cs * menuScale + ")";
			// socialCard.style["margin"] = (cs * 10) + "px";
			// socialCard.style["margin-bottom"] = (cs * 18) + "px";
			cs *= serverScale;
		}
		...
		Unit.prototype.renderTop = function() {
			...
			if (debug) {
				game.fillStyle = "#f00";
				game.beginPath();
				game.arc(fx(this.$x), fy(this.$y), 10, 0, 2*Math.PI);
				game.fill();
				game.closePath();
			}
		}
		...
		if (mobile) {
			...
			// linksBar.style.left = "auto";
			// linksBar.style.right = "0px";
			// linksBar.style.transform = "none";
			// var split = linksBar.innerHTML.split("Privacy</a> | ");
			// if (split.length > 1) {
			// 	linksBar.innerHTML = linksBar.innerHTML.split("Privacy</a> | ")[1];
			// }
			// partyElement.style.right = "inherit";
			// partyElement.style.left = "0px";
			middleContainer.style.transform = "translate(-50%, -50%) scale(" + mobileScale + ")";
			appsCard.style.transform = "scale(" + mobileScale + ")";
			appsCard.style.margin = (8 * mobileScale) + "px";
			appsCard.style.bottom = (92 * mobileScale + 8 * mobileScale) + "px";
			partyElement.style.transform = "scale(" + mobileScale + ")";
			partyElement.style.margin = (8 * mobileScale) + "px";
			partyElement.style.bottom = (8 * mobileScale) + "px";
			playerCountElement.style.transform = "scale(" + mobileScale + ")";
			playerCountElement.style.margin = (8 * mobileScale) + "px";
			versionElement.style.transform = "scale(" + mobileScale + ")";
			versionElement.style.margin = (8 * mobileScale) + "px";
			storeOuter.style.width = "584px";
			storeOuter.style.transform = "translateX(-50%) scale(" + mobileScale + ")";
			storeOuter.style.margin = (8 * mobileScale) + "px";
			storeBar.style.width = "504px";
			storeToggle.style.transform = "translateX(-100%) scale(" + mobileScale + ")";
			storeToggle.style.margin = (8 * mobileScale) + "px";
			storeToggle.style.left = "calc(50% - " + (292 * mobileScale) + "px - " + (8 * mobileScale) + "px)";
			stashElement.style.transform = "scale(" + mobileScale + ")";
			stashElement.style.margin = (8 * mobileScale) + "px";
			stashMainBox.style.margin = "0px";
			stashSpecialBox.style.margin = "0px";
			stashSpecialBox.style.marginBottom = 8 + "px";
			unitCounter.style.transform = "scale(" + mobileScale + ")";
			unitCounter.style.margin = (8 * mobileScale) + "px";
			leaderboardBox.style.transform = "scale(" + mobileScale + ")";
			leaderboardBox.style.margin = (8 * mobileScale) + "px";
			scoreBox.style.transform = "scale(" + mobileScale + ")";
			scoreBox.style.margin = (8 * mobileScale) + "px";
			alertsElement.style.transform = "translateX(-50%) scale(" + mobileScale + ")";
			alertsElement.style.bottom = (156 * mobileScale) + "px";
			optionsBox.style.top = (8 * mobileScale) + "px";
			optionsBox.style.transform = "translateX(-50%) scale(" + mobileScale + ")";

			helpBox.style.position = "fixed";
			helpBox.style.left = "50%";
			helpBox.style.top = "-200%";
			helpBox.style.transform = "translate(-50%, -50%) scale(" + mobileScale * 1.5 + ")";
		...
		var update = function() {
			...
			if (debug) {
				...
				game.fillStyle = "#f00";
				game.beginPath();
				game.arc(fx(0), fy(0), 10, 0, 2*Math.PI);
				game.fill();
				game.closePath();
			}
		...
		function populateStore() {
			...
			for (var i = 0; i < materialNum - 1; i++) {
				for (var j = 0; j < unitTypesSorted.length; j++) {
					if (unitTypesSorted[j] != unitType.core) {
