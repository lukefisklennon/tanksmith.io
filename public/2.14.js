...
var go = function(ip, mode) {
	try {
		...
		var keysCodes = [87, 38, 65, 37, 83, 40, 68, 39, 1, 2, 81, 69];
		var keyStates = {};
		...
		function resizeContent() {
			...
			if (parse_query_string(window.location.search.substring(1)).source != "app") {
				if (mobile) {
					...
					if (g("tanksmith-io_728x90")) {
						g("tanksmith-io_728x90").style.transform = "translateX(-50%) scale(" + gameScale + ")";
						g("tanksmith-io_728x90").style.transformOrigin = "50% 0%";
						g("tanksmith-io_728x90").style.top = (8 * gameScale) + "px";
					}
					...
					switch(code) {
						...
						case serverCode.die:
							...
							if (!debug && parse_query_string(window.location.search.substring(1)).source != "app") {
								...
								if (g("tanksmith-io_728x90")) {
									aiptag.cmd.display.push(function() { aipDisplayTag.display('tanksmith-io_728x90'); });
								}
							}
							var nameLength = view.getUint8(offset, true);
							offset += byte;
		...
		function joined() {
			setTimeout(function() {
				...
				if (storage.tutorialStage == tutorial.wasd) {
					...
					storage.tutorialStage++;
					saveStorage();
				} else if (storage.tutorialStage > tutorial.gotUnit && mode == "ffa") {
					pushMessage("You’re protected from damage until build your first unit", longMessage * 2);
				}
			}, serverTargetDelta);
		}
		...
		var itemClick = function(e) {
			...
			if (!placing || placingType != Number(parts[0]) || placingMaterial != Number(parts[1])) {
		...
		function populateStore() {
			...
			for (var i = 0; i < materialNum - 1; i++) {
				for (var j = 0; j < unitTypesSorted.length; j++) {
					if (unitTypesSorted[j] != unitType.core && (window.location.hostname == "localhost" || unitTypesSorted[j] != unitType.boosterUnit)) {
