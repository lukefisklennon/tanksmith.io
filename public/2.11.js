...
var go = function(ip) {
	try {
		...
		var debug = false;//(window.location.hostname == "localhost");
		var dataTotal = 0;
		var dataInPeriod = 0;
		var dataPerPeriod = 0;
		var fps = 0;
		...
		var itemClick = function(e) {
			...
			if (linearScale(unitTypes[placingType].cost, placingMaterial) <= $stash[placingMaterial] && myUnits.length <= unitLimit) {
				placing = true;
				if (mobile) {
					joystickDown = false;
					targetDown = false;
